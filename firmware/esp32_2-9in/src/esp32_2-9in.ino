...
const char* certificate_rawcontent = rsb_root;  // ok, should work until Fri, 14 Mar 2025 23:59:59 GMT
...
void setup()
{
  ...
  // WiFiManagerParameter custom_token("API Token", "Enter token here", api_token, 60);
  custom_token = new WiFiManagerParameter("api_token", "RSB API Token (visit rsb.nz to get!)", api_token, 64);
  
  wm.addParameter(custom_token);
  wm.setAPCallback(configModeCallback);
  // res = wm.autoConnect("RSB-Device","RSB-Paper"); // password protected ap
  res = wm.autoConnect("RSB-Device"); // anonymous ap
